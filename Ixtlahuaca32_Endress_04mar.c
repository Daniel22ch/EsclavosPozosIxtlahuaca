...
//Para debug se pone <#define PRINTFDEBUG printf> para que no haya debug se pone <#define PRINTFDEBUG //printf>
  ...
  //#define DEBUG
  #define			addr_HMI							5
  #define        Numero_sitio               7                                   // WEB // TODO: Poner el Numero_sitio correspondiente al Scairlink
  ...
  #define           reg_totalizado         2609	// TODO: Poner el registro del medidos en donde esta el reg_totalizado
  ...
  const unsigned char Version[]=				"Version 30.0";
  const unsigned char Fecha_Ver[]=				"13/nov/2020";
   ...
   my4XRegs[ addr_reg_mb_ctrl_web]			= 0;
    ...
    costate{
          ...
          waitfor(DelayMs(100));
            ...
            waitfor(DelayMs(100));
                          // it  = mmRead_clon(     unsigned wAddr,      unsigned wReg,   unsigned wCount,                  void *pwRegs)
            waitfor (      error = mmRead_clon(  dir_medidor_flujo,   						reg_totalizado,                 2,                   (int *)&bufauxint[0]));
            memcpy(&bufaux[0], &bufauxint[0],4);
            dato32b.to_byte[0]=bufaux[0];
            dato32b.to_byte[1]=bufaux[1];
            dato32b.to_byte[2]=bufaux[2];
            dato32b.to_byte[3]=bufaux[3];
            //PRINTFDEBUG("Orden1: %f", dato32b.to_float);
            /*dato32b.to_byte[0]=bufaux[1];
            dato32b.to_byte[1]=bufaux[0];
            dato32b.to_byte[2]=bufaux[3];
            dato32b.to_byte[3]=bufaux[2];
            PRINTFDEBUG("Orden2: %f", dato32b.to_float);
            /*dato32b.to_byte[0]=bufaux[3];
            dato32b.to_byte[1]=bufaux[2];
            dato32b.to_byte[2]=bufaux[1];
            dato32b.to_byte[3]=bufaux[0];
            PRINTFDEBUG("Orden3: %f", dato32b.to_float);
            /*dato32b.to_byte[0]=bufaux[2];
            dato32b.to_byte[1]=bufaux[3];
            dato32b.to_byte[2]=bufaux[0];
            dato32b.to_byte[3]=bufaux[1];
            PRINTFDEBUG("Orden4: %f", dato32b.to_float);*/


            my4XRegs[110]=dato32b.to_int16[1];
            my4XRegs[111]=dato32b.to_int16[0];
            if( error == 0xffff ){
