...
Ver 30-31 Fue definitiva durante Dic2020- Marzo 2021
Ver 33.0 se añade enlace tipo directo, indirecto, ambos, ninguno, tabla WR by database es 2300+tabla normal My4xregs=2425 // Control de comandos via HMI con perilla en local
*/
...
//Para debug se pone <#define PRINTFDEBUG printf> para que no haya debug se pone <#define PRINTFDEBUG //printf>
  ...
  //#define DEBUG
  #define			addr_HMI							5
  #define        Numero_sitio               15                                   // WEB // TODO: Poner el Numero_sitio correspondiente al Scairlink
  #define        dir_scairlink              100 + Numero_sitio                  // VO 1 SCAIRLINK
  #define			dir_medidor_flujo				247  // TODO: Poner la dir_medidor_flujo correspondiente
  #define 			reg_flujo              2007 // TODO: Poner el registro del medidor en dodne esta el flujo inst
  #define           reg_totalizado         2610	// TODO: Poner el registro del medidos en donde esta el reg_totalizado
  #define        inicio_tab_hmi             100
  #define TIMEOUT_ESTADOWEB                   300 // timeout en segundos para el enalce web
  #define WRCONTADORENLACEWEB  2425	
  ...
  const unsigned char Fecha_Ver[]=				"04/mar/2021";
...
/////////////////////////////////////////////////////////////////////////////////
  ...
  #define 			reset_router_cel				18
  #define 			reset_scairlink				19
  #define			reset_rabbit					20												// RELE 2
...
/*===================================================================
  ENLACE DIRECTO E INDIRECTO
===================================================================*/
  unsigned int enlaceweb_cont;
  unsigned char enlaceweb_estado;
  unsigned long enlaceweb_timerweb;
  unsigned char flag_inicio;

  unsigned int calcula_estado_enlaceweb(void){
    if(enlaceweb_cont==my4XRegs[WRCONTADORENLACEWEB]){// no ha camabiado el contador
      if(read_rtc() - enlaceweb_timerweb > TIMEOUT_ESTADOWEB || flag_inicio==1){// ya pasó el timeout
        flag_inicio=0;
        enlaceweb_cont=my4XRegs[WRCONTADORENLACEWEB];
        enlaceweb_timerweb=read_rtc();
        enlaceweb_estado=0x00;//el estado web se pone en 0 SOLO si no cambio el contador web y ya paso el timeout
        return 0;
      }else{
        return 0;
      }
    }else{// ya cambio el contador
      enlaceweb_cont=my4XRegs[WRCONTADORENLACEWEB];
      enlaceweb_timerweb=read_rtc();
      enlaceweb_estado=0x01;// el estado web se pone en 1 SOLO si cambio el contador 
      return 1;
    }
  }  
/*===================================================================
...
main()
{
  ...
  ===================================================================*/
     ...
     setDigOut(reset_router_cel, 1);
     setDigOut(reset_scairlink, 1);
     setDigOut(reset_rabbit,1);
  ...
  ***************************************************************************************************************/
  ...
  enlaceweb_estado=0;
  enlaceweb_timerweb=read_rtc();
  my4XRegs[WRCONTADORENLACEWEB]=0;
  flag_inicio=1;
    ...
    costate{
          ...
          if( conta_seg >= 4 ){
              ...
              if( error == 0xffff ){
                if( my4XRegs[enlace_scairlink] >= 2){
                  calcula_estado_enlaceweb();
                  if(enlaceweb_estado==0x01){//rf ok + web ok                  
                    my4XRegs[enlace_scairlink]=0x03;
                  }else{ // rf bien + web mal
                    my4XRegs[enlace_scairlink]=0x01;
                  }
                }else{
                  calcula_estado_enlaceweb();
                  if(enlaceweb_estado==0x01){//rf mal + web ok                  
                    my4XRegs[enlace_scairlink]=0x02;
                  }else{ // rf mal + web mal
                    my4XRegs[enlace_scairlink]=0x00;
                  }
                }
              }else{
                PRINTFDEBUG("ERR RD Enlace Scairlink\n");
                calcula_estado_enlaceweb();
                if(enlaceweb_estado==0x01){//rf mal + web ok                  
                  my4XRegs[enlace_scairlink]=0x02;
                }else{ // rf mal + web mal
              ...
              if( flag_inicio_control_A == 0 ){
               ...
               {
                  if( my4XRegs[status_perilla] == 1 || (my4XRegs[status_perilla] == 2 && cmd_control_hmi != 0)){   // Hay un comando control y perilla en automatico o perilla en local y comando hmi
          ...
          waitfor(DelayMs(100));
            ...
            waitfor (      error = mmRead_clon(  dir_medidor_flujo,   						reg_flujo,                 2,                   (int *)&my4XRegs[105]));
            ...
            waitfor(DelayMs(100));
                          // it  = mmRead_clon(     unsigned wAddr,      unsigned wReg,   unsigned wCount,                  void *pwRegs)
            waitfor (      error = mmRead_clon(  dir_medidor_flujo,   						reg_totalizado,                 2,                   (int *)&my4XRegs[110]));

            if( error == 0xffff ){
                    my4XRegs[113]=1; // se pone 1 en rango si respondio
                    my4XRegs[114]=1;
            }else{
                    PRINTFDEBUG("Error WR Med Flujo\n");
          ...
          #else
            conver.conver_float 	= 	(float) totalizador*CONF_ANA[4].K2_data;
            PRINTFDEBUG("totalizado= %f", conver.conver_float);
